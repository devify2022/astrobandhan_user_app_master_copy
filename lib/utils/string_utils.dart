...
class StringUtils {
  ...
  dynamic hasTheRightLengthCase(String? value,int theRightLength,{bool returnBool=false}) {
    if (value==null||value.isEmpty||value.length <theRightLength){
      return returnBool?false:"Must be at least $theRightLength characters in length";
    }
    return returnBool?true:null;
  }
  ...
  String? isValidPassword(String? value) {
    if(!(hasTheRightLengthCase(value,8,returnBool: true) as bool)){
      return AppLocalizations.of(context)!.translate("must be at least 8 characters in length",replacement: "Password");
    }else if(!(hasUpperCase(value!,returnBool: true) as bool)){
     return AppLocalizations.of(context)!.translate("should contain at least one upper case",replacement: "Password");
    }else if(!(hasLowerCase(value,returnBool: true) as bool)){
      return AppLocalizations.of(context)!.translate("should contain at least one lower case",replacement: "Password");
    }else if(!(hasDigitCase(value,returnBool: true) as bool)){
      return AppLocalizations.of(context)!.translate("should contain at least one digit",replacement: "Password");
    }else if(!(hasSpecialCharacterCase(value,returnBool: true) as bool)){
      return AppLocalizations.of(context)!.translate("should contain at least one Special character",replacement: "Password");
    }
    return null;
  }

  String? isValidConfirmPasswordPassword(String? password,String? confirmPassword) {
    if(password!=confirmPassword) {
      return AppLocalizations.of(context)!.translate("confirm password is not matched");
    }
    return null;
  }
  ...
  static Size measure(String text, TextStyle style, {int maxLines= 1, TextDirection direction = TextDirection.ltr, double? maxWidth}) {
    final TextPainter textPainter = TextPainter(text: TextSpan(text: text, style: style), maxLines: maxLines, textDirection: direction)..layout(maxWidth: maxWidth ?? double.infinity);
...
extension CapExtension on String {
  String get inCaps => this.isNotEmpty ?'${this[0].toUpperCase()}${substring(1)}':'';
